...
### REST Endpoints
- `POST /upload` — Upload CSV dataset (returns `file_id`, row/column count, column names)
- `GET /preview/{file_id}` — Get first 5 rows of dataset
- `GET /health` — Health status check

### WebSocket (`ws://localhost:8000/ws/analyze/{file_id}`)
- **Client payload:** `{"query": "Analyze correlation between income and spending"}`
- **Server events:**
  - `status` — Status text ("Agent is thinking...")
  - `thought` — Tool usage decision ("Using tool: python_repl")
  - `code` — Python code generated by the agent
  - `output` — Textual stdout / statistical results
  - `chart` — Base64 PNG image string
  - `answer` — Final Markdown report
